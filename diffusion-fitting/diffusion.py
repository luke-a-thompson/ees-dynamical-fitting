...
pots = ff.createPotential(
    pdb.topology, nonbondedMethod=CutoffPeriodic, nonbondedCutoff=rc * nanometer
)
...
def nvt_sample(init_stru, parameter, trajectory, key):
    ...
    integrator = LangevinMiddleIntegrator(
        T * kelvin, 0.1 / picosecond, dt * picoseconds
    )
    ...
    state_init["vel"] = (
        jnp.einsum(
            "ijk,j->ijk",
            random.normal(subkey, shape=positions.shape),
            jnp.sqrt(kT / mass * 1e3),
        )
        / 1e3
    )
...
def L(traj, params):
    def l_out(traj, params):
        target_energy_function = buildTrajEnergyFunction(
            efunc, cov_map, rc, ensemble="nvt", useFreud=True
        )
...
def nve_reweight(state_init, params):
    ...
    Loss = Generator.generate_Loss(
        partial(L, params=params), has_aux=True, metadata=metadata
    )
...
multiTrans["NonbondedForce/sigma"] = genOptimizer(
    learning_rate=0.0002, clip=0.001, nonzero=False
)
multiTrans["NonbondedForce/epsilon"] = genOptimizer(
    learning_rate=0.01, clip=0.05, nonzero=False
)
multiTrans.finalize()
...
for nloop in range(1, nloops):
    ...
    updates, opt_state = grad_transform.update(
        gradient_choose, opt_state, params=params_choose
    )
