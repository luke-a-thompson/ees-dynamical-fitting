...
class BaseDriver(object):
    ...
    def init(self):
        ...
        """
        self.nbead = np.frombuffer(self.socket.recv(INT * 1), dtype=np.int32)[0]
    ...
    def posdata(self):
        ...
        Read position data.
        """
        self.cell = np.frombuffer(self.socket.recv(FLOAT * 9), dtype=np.float64) * BOHR
        self.inverse = (
            np.frombuffer(self.socket.recv(FLOAT * 9), dtype=np.float64) / BOHR
        )
        self.natom = np.frombuffer(self.socket.recv(INT * 1), dtype=np.int32)[0]
        if self.socket_type == "unix":
            crd = np.frombuffer(
                self.socket.recv(FLOAT * 3 * self.natom, socket.MSG_WAITALL),
                dtype=np.float64,
            )
        else:
            crd = np.frombuffer(
                self.socket.recv(FLOAT * 3 * self.natom), dtype=np.float64
            )
...
class GaussDriver(BaseDriver):
    ...
    def readlog(self):
        ...
        forces = [
            [np.float64(i[0]), np.float64(i[1]), np.float64(i[2])] for i in forces
        ]
