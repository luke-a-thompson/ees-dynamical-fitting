...
def _cell_dimensions(
    ...
    cells_per_side = onp.array(cells_per_side, dtype=i64)
...
def count_cell_filling(
    ...
    particle_index = jnp.array(position / cell_size, dtype=i64)
...
def _compute_hash_constants(spatial_dimension: int, cells_per_side: Array) -> Array:
    if cells_per_side.size == 1:
        return jnp.array(
            [[cells_per_side**d for d in range(spatial_dimension)]], dtype=i64
        )
    elif cells_per_side.size == spatial_dimension:
        one = jnp.array([[1]], dtype=i64)
        cells_per_side = jnp.concatenate((one, cells_per_side[:, :-1]), axis=1)
        return jnp.array(jnp.cumprod(cells_per_side), dtype=i64)
    else:
        raise ValueError()
...
def _neighboring_cells(dimension: int) -> Generator[onp.ndarray, None, None]:
    for dindex in onp.ndindex(*([3] * dimension)):
        yield onp.array(dindex, dtype=i64) - 1
...
def cell_list(
    ...
    def cell_list_fn(
        ...
        **kwargs,
        ...
        cell_id = N * jnp.ones((cell_count * cell_capacity, 1), dtype=i64)
        ...
        indices = jnp.array(position / cell_size, dtype=i64)
        ...
        sorted_cell_id = jnp.mod(lax.iota(i64, N), cell_capacity)
...
def neighbor_list(
    ...
    **static_kwargs,
    ...
    if fractional_coordinates:
        ...
        box_size = (
            f32(box_size) if onp.isscalar(box_size) else onp.ones_like(box_size, f32)
        )
    ...
    def cell_list_candidate_fn(cl: CellList, position: Array) -> Array:
        ...
        neighbor_idx = jnp.zeros((N + 1,) + cell_idx.shape[-2:], i64)
    ...
    def mask_self_fn(idx: Array) -> Array:
        self_mask = idx == jnp.reshape(
            jnp.arange(idx.shape[0], dtype=i64), (idx.shape[0], 1)
        )
    ...
    def prune_neighbor_list_dense(position: Array, idx: Array, **kwargs) -> Array:
        ...
        out_idx = N * jnp.ones(idx.shape, i64)
    ...
    def prune_neighbor_list_sparse(position: Array, idx: Array, **kwargs) -> Array:
        ...
        out_idx = N * jnp.ones(receiver_idx.shape, i64)
...
def neighbor_list_mask(neighbor: NeighborList, mask_self: bool = False) -> Array:
    ...
    if mask_self:
        ...
        self_mask = neighbor.idx != jnp.reshape(jnp.arange(N, dtype=i64), (N, 1))
        mask = mask & self_mask
    return mask
...
def to_jraph(
    ...
    if mask is not None:
        ...
        ordered = N * jnp.ones((len(receivers) + 1,), i64)
