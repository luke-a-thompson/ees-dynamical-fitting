...
class DMFFDriver(driver.BaseDriver):
    def __init__(self, addr, port, pdb, f_xml, r_xml, psr, socktype, device="cpu"):
        ...
        def admp_calculator(positions, L):
            ...
            E_eann = jnp.array(
                eann_force.get_energy(positions, box, pairs_b, params_eann)
            )
    ...
    def grad(self, crd, cell):  # receive SI input, return SI values
        ...
        # convert to SI
        energy = np.array(energy * 1000 / 6.0221409e23)  # kj/mol to Joules
        grad = np.array(
            grad * 1000 / 6.0221409e23 * 1e10
        )  # convert kj/mol/A to joule/m
        virial = np.array(virial * 1000 / 6.0221409e23)  # kj/mol to Joules
...
if __name__ == "__main__":
    ...
    fn_pdb = sys.argv[
        1
    ]  # pdb file used to define openmm topology, this one should contain all virtual sites
