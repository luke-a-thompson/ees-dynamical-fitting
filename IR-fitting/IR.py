...
from difftraj_ees import nve_reweight_ees as _nve_reweight_ees, EES25
from eann import EANNForce
from scipy.interpolate import interp1d

USE_EES = True  # Set False to use original leapfrog Verlet
...
def efunc2(pos, box, pairs, params):
    return jnp.array(
        eann_force2.get_energy(pos * 10, box * 10, pairs, params["energy"])
    )
...
def make_dipole(pos, box, dipole_model):
    ...
    for i in range(len(pos) // 40):
        wannier, wannier_grad, _ = dipole_model.eval_full(
            pos[40 * i : 40 * (i + 1)] * 10, box[40 * i : 40 * (i + 1)] * 10, atype
        )
    ...
    dipole_grad[:, 0, :, 0] = np.tile(
        np.tile([-2, 1, 1], natoms1 // 3).reshape([1, natoms1]), (len(pos), 1)
    )
    dipole_grad[:, 1, :, 1] = np.tile(
        np.tile([-2, 1, 1], natoms1 // 3).reshape([1, natoms1]), (len(pos), 1)
    )
    dipole_grad[:, 2, :, 2] = np.tile(
        np.tile([-2, 1, 1], natoms1 // 3).reshape([1, natoms1]), (len(pos), 1)
    )
...
def L(traj):
    ...
    corr = vmap(calculate_corr_vdipole, in_axes=(1, None, None))(
        dipoles, dt, corr_length
    )
    ...
    return 10 * jnp.sum((lineshape[id1] - lineshape_ref1) ** 2) + jnp.sum(
        (lineshape[id2] - lineshape_ref2) ** 2
    ), (
        jnp.concatenate([wavenum[id1], wavenum[id2]]),
        jnp.concatenate([lineshape_ref1, lineshape_ref2]),
        jnp.concatenate([lineshape[id1], lineshape[id2]]),
    )


def nve_reweight(state_init, params):
    Generator = Loss_Generator(
        f_nout, box1, state_init["pos"][0], mass, dt, nsteps, nout, cov_map1, rc, efunc1
    )
    state = {}
    state["pos"] = jnp.concatenate(
        [
            vmap(Generator.regularize_pos)(state_init["pos"] - state_init["vel"] * dt),
            state_init["pos"],
        ]
    )
    state["vel"] = jnp.concatenate([-state_init["vel"], state_init["vel"]])
    state_splits = [
        {key: state[key][i * batch_size : (i + 1) * batch_size] for key in state}
        for i in range(len(state["vel"]) // batch_size)
    ]
    traj = []
    final_states = []
    ...
    (err, (wavenum, lineshape_ref, lineshape)), gradient_traj = value_and_grad(
        L, has_aux=True
    )(traj)
    gradient_trajs = jnp.array_split(
        gradient_traj, len(state["vel"]) // batch_size, axis=1
    )
...
def nve_reweight_ees_wrapper(state_init, params):
    return _nve_reweight_ees(
        state_init,
        params,
        f_nout=f_nout,
        box=box1,
...
def Loss2(params, density):
    target_energy_function = buildTrajEnergyFunction(
        efunc2, cov_map2, rc, ensemble="npt", useFreud=True
    )
...
multiTrans1["energy/inta"] = genOptimizer(
    learning_rate=0.00005, clip=0.01, nonzero=False
)
multiTrans1["energy/initpot"] = genOptimizer(
    learning_rate=0.00005, clip=0.01, nonzero=False
)
...
for nloop in range(nloops):
    if len(estimator.states) < 1:
        ...
        state = OpenMMSampleState(
            state_name, potential2, params, temperature=T, pressure=1.0
        )
    ...
    (loss2, (density, utarget)), gradient2 = value_and_grad(
        Loss2, argnums=(0), has_aux=True
    )(params, density_ref)
    ...
    if len(estimator.states) >= 1:
        ...
        if USE_EES:
            loss1, gradient1, wavenum, lineshape_ref, lineshape = (
                nve_reweight_ees_wrapper(state_init, params)
            )
        else:
            loss1, gradient1, wavenum, lineshape_ref, lineshape = nve_reweight(
                state_init, params
            )
        ...
        updates, opt_state1 = grad_transform1.update(
            gradient, opt_state1, params=params
        )
