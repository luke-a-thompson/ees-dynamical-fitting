...
class NVT_langevin:
    def __init__(self, init_stru, potential):
        ...
        self.m = jnp.repeat(
            jnp.tile(jnp.array([[m_O, m_H, m_H]]), self.natoms // 3), 3, axis=0
        ).T
        idh = jnp.delete(
            jnp.arange(0, self.natoms), jnp.arange(0, self.natoms // 3) * 3
        )
    ...
    def get_init_state(self, T, key, pos=None):
        ...
        if pos == None:
            ...
            state["pos"] = pos
        state["vel"] = (
            random.normal(key, shape=(self.natoms, 3))
            * jnp.sqrt(kT / self.m * 1e3)
            / 1e3
        )
        return state
    ...
    def nvt_nout(self, state, params, key):
        ...
        state, traj = jax.lax.fori_loop(
            0, self.nsteps // self.nout, ode_fwd, (state, traj)
        )
        return traj
    ...
    def run_nvt(self, state, params, key, logfile, dcdfile):
        ...
        uu = u
        print(
            '#"Time (ps)","Potential Energy (kJ/mole)","Kinetic Energy (kJ/mole)","Total Energy (kJ/mole)","Temperature (K)"',
            file=ifile,
        )
        subkeys = random.split(key, num=self.nsteps // self.nout)
        for i in range(self.nsteps // self.nout):
            ...
            temp = (jnp.std(state["vel"] * jnp.sqrt(self.m), ddof=-1) * 1e3) ** 2 / (
                1e3 * KB * NA
            )
            print(
                "%s, %s, %s, %s, %s"
                % (self.nout * self.dt * i, potE, kinE, potE + kinE, temp),
                file=ifile,
            )
            state = self.vv_nout(state, params, subkeys[i])
...
def apply_gussian_filter(corr: jnp.ndarray, width: float):
    ...
    """
    nmax = corr.shape[0] - 1
    return corr * jnp.exp(-0.5 * (0.5 * width * jnp.arange(nmax + 1) / nmax) ** 2)
...
def FT(DT: float, C: jnp.ndarray, M: Optional[int] = None) -> jnp.ndarray:
    ...
    BETA = BETA.at[0].set(2.0 / 3.0)
    GAMMA = GAMMA.at[0].set(4.0 / 3.0)
